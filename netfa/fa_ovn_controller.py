...
class OvnController(FaSdnController):
    ...
    def register_vnid(self, req):
        ...
        port_name = self._add_patch_port(self.CONF.controller_br_name, req.vNID, 'fa-br')

        port_uuid = uuid.uuid4()

        # add port to ovnnb
        os.system('ovn-nbctl -d tcp:10.0.2.6:6640 lport-add neutron-%s %s' % (req.vNID, port_uuid))  #XXX
        ...
        port = self._add_patch_port(self.CONF.fa_br_name, 'fa-br', req.vNID)
